...
using namespace std;
using gvirtus::communicators::MqttCommunicator;

MqttCommunicator::MqttCommunicator(const std::string &communicator) {
#ifdef _WIN32
    if (!initialized) {
        WSADATA data;
    ...
    free(hostname);
    if (ent == NULL)
        throw runtime_error("MqttCommunicator: Can't resolve hostname '" + mHostname + "'.");
    mInAddrSize = ent->h_length;
    mInAddr = new char[mInAddrSize];
    memcpy(mInAddr, *ent->h_addr_list, mInAddrSize);
}

MqttCommunicator::MqttCommunicator(const char *hostname, short port) {
    mHostname = string(hostname);
    struct hostent *ent = gethostbyname(hostname);
    if (ent == NULL)
        throw runtime_error("MqttCommunicator: Can't resolve hostname '" + mHostname + "'.");
...
MqttCommunicator::MqttCommunicator(int fd, const char *hostname) {
    mSocketFd = fd;
    InitializeStream();
}
...
void MqttCommunicator::Serve() {
#ifdef DEBUG
    printf("MqttCommunicator::Serve() called\n");
#endif

    struct sockaddr_in socket_addr;

    if ((mSocketFd = socket(AF_INET, SOCK_STREAM, 0)) == 0)
        throw runtime_error("MqttCommunicator: Can't create socket: " + string(strerror(errno)) +
                            ".");
    ...
    if (bindResult != 0)
        throw runtime_error("MqttCommunicator: Can't bind socket: " + string(strerror(errno)) + ".");

    int listenResult = listen(mSocketFd, 5);
    if (listenResult != 0)
        throw runtime_error(
            "MqttCommunicator: Can't listen from socket: " + string(strerror(errno)) + ".");

#ifdef DEBUG
    cout << "MqttCommunicator::Serve() returned" << endl;
#endif
}

const gvirtus::communicators::Communicator *const MqttCommunicator::Accept() const {
#ifdef DEBUG
    cout << "MqttCommunicator::Accept() called" << endl;
#endif
...
#ifdef DEBUG
    cout << "MqttCommunicator::Accept() client_socket_fd: " << client_socket_fd << endl;
#endif
    return new MqttCommunicator(client_socket_fd, inet_ntoa(client_socket_addr.sin_addr));
}

void MqttCommunicator::Connect() {
#ifdef DEBUG
    cout << "MqttCommunicator::Connect() called " < < < < endl;
#endif
    ...
    if (connect(mSocketFd, (struct sockaddr *)&remote, sizeof(struct sockaddr_in)) != 0)
        throw runtime_error("MqttCommunicator: Can't connect to socket: " + string(strerror(errno)) +
                            ".");

    InitializeStream();

#ifdef DEBUG
    cout << "MqttCommunicator::Connect() returned" << endl;
#endif
}

void MqttCommunicator::Close() {}

size_t MqttCommunicator::Read(char *buffer, size_t size) {
#ifdef DEBUG
    cout << "MqttCommunicator::Read() size: " << size << endl;
#endif
...
#ifdef DEBUG
    cout << "MqttCommunicator::Read() returned " << ret_value << endl;
#endif

    return ret_value;
}

size_t MqttCommunicator::Write(const char *buffer, size_t size) {
#ifdef DEBUG
    cout << "MqttCommunicator::Write() called" << endl;
#endif
...
#ifdef DEBUG
    cout << "MqttCommunicator::Write() returned" << size << endl;
#endif

    return size;
}

void MqttCommunicator::Sync() { mpOutput->flush(); }

void MqttCommunicator::InitializeStream() {
#ifdef _WIN32
...
extern "C" std::shared_ptr<MqttCommunicator> create_communicator(
    ...
    return std::make_shared<MqttCommunicator>(arg);
}
