...
class CommunicatorFactory {
   public:
    ...
    get_communicator(std::shared_ptr<Endpoint> end, bool secure = false) {
        ...
        std::cout << "CommunicatorFactory::get_communicator(): endpoint is: " << end->to_string()
                  << std::endl;
#endif
        ...
        std::vector<std::string> unsecureMatches = {
            "tcp", "http", "oldtcp", "ws", "ib", "hybrid", "udp",
        ...
        if (not secure) {
            ...
            auto foundIt =
                std::find(unsecureMatches.begin(), unsecureMatches.end(), end->protocol());
...
#ifdef DEBUG
        std::cout << "CommunicatorFactory::get_communicator(): found protocol: " << end->protocol()
                  << std::endl;
#endif
...
#ifdef DEBUG
        std::cout << "CommunicatorFactory::get_communicator(): dl_string: " << dl_string
                  << std::endl;
#endif
